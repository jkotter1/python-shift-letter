...
class mywindow(QtWidgets.QMainWindow):
    ...
    #            #self.load_most_recently_created_record()
    ...
    def check_if_refresh_needed(self):
        ...
        letterID = self.ui.LetterID.toPlainText()
        for fieldname in self.ui.LatestValueDict: #w in self.allWidgets():
            #print(fieldname)
            fname = str(fieldname)
            fieldval = self.ui.LatestValueDict[fname]
            #print(fieldval)
            #fieldname = w.widget().objectName()
            #fieldtype = w.widget().__class__.__name__
            #if fieldtype == "QTextEdit" or fieldtype == "QDateEdit" or fieldtype == "QComboBox":  #Need to start using the Last Loaded Dictionary entries for fieldval, not the current field values (since they WILL be different if edited by the user.
                #if fieldtype == "QTextEdit":
                #    fieldval = w.widget().toPlainText()
                #elif fieldtype == "QDateEdit":
                #    fieldval = str(w.widget().date())
                #elif fieldtype == "QComboBox":
                #    fieldval = str(w.widget().currentText())
            dbval = self.query_latest_record(fieldname, letterID)
            if dbval != fieldval:
                print("Update needed")
                #print (fieldval)
                #print(dbval, fieldval)
