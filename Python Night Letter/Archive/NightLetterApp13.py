    ...
    def getFilestoRemove(self):
        ...
        for i in range(layout.count()):
            ...
            if checkBox.isChecked():
                ...
                #print(attachString)
                #splitFiles = attachFieldString.split("<p ")
                #for index, string in splitFiles:
                #    if string[0] == "s":
                #        print ("changing")
                #        splitFiles[index] = "<p " + string
                #print(splitFiles)
                #for fileChunk in splitFiles:
                #    print(fileChunk)
                #    if fileChunk.find(cbName) != -1:
                #        htmlToRemove = fileChunk.rsplit("</body>")[0] #if this was the last item on the list, cut the </body> and </html> tags so they aren't removed. This is for you, W3 Schools! We love you!
                #        print("Removing\n")
                #        print(htmlToRemove)
                #        newAttachFieldString = attachFieldString.replace(htmlToRemove,"") #remove the <p> tag with the file that needs to be removed.
                #        print(newAttachFieldString)
